...
})
{
  ...
  return (
    <div className={`${state ? "fixed" : "hidden"} bg-black/40 backdrop-blur-xs w-full h-full top-0 left-0 z-100`}>
      <div className={`absolute flex flex-col w-fit h-fit top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white rounded-lg p-10 z-100`}> {/*Contact Card*/ }
        <span 
          className="absolute top-0 right-0 translate-x-1/2 -translate-y-1/2 bg-(--dark-blue) w-7 h-7 text-white text-center rounded-4xl hover:cursor-pointer"
          onClick={() => {handler()}}
        >X</span> {/*Close icon*/}
